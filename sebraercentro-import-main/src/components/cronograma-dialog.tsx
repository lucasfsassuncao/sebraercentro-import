...
import {
  ETAPAS, MAX_HORAS_DIA, parseISO, proximoAposDiaUtil, proximoDiaUtil, toISO, totalHoras, type Etapa,
} from "@/lib/horas";
import {
  obterFeriados,
  gerarLinhasParaEmpresa,
...
export function CronogramaDialog({
  ...
  useEffect(() => {
    ...
    async function loadFeriados() {
      ...
      if (u.user) {
        const f = await obterFeriados(u.user.id);
        setFeriados(f);
      }
    }
    loadFeriados();
  ...
  // Gera cronograma inicial usando o motor inteligente
  useEffect(() => {
    if (!open) return;
    const inicial: Record<string, Linha[]> = {};
    for (const e of alvos) {
      const linhasGeradas = gerarLinhasParaEmpresa(projeto, e, feriados);
      inicial[e.id] = linhasGeradas.map((l) => ({
        data: l.data,
        horas: l.horas,
        etapa: l.etapa,
  ...
  function adicionarLinha(empresaId: string) {
    setPorEmpresa((s) => {
      ...
      const prox = ultima 
        ? toISO(proximoAposDiaUtil(parseISO(ultima), feriados)) 
        : toISO(proximoDiaUtil(new Date(), feriados));
  ...
  async function salvar() {
    ...
    // validações usando o motor inteligente
    for (const empresaId of Object.keys(porEmpresa)) {
      ...
      const modeloEfetivo = e.modelo || projeto.modelo;
    ...
    for (const empresaId of Object.keys(porEmpresa)) {
      ...
      arr.forEach((l, idx) => {
        linhas.push({
          ...
          municipio: e.municipio || projeto.municipio,
          codigo_ibge: e.codigo_ibge || projeto.codigo_ibge,
          codigo_tema: e.codigo_tema || projeto.codigo_tema,
          codigo_categoria: e.codigo_categoria || projeto.codigo_categoria,
          codigo_meio_atendimento: e.codigo_meio_atendimento || projeto.codigo_meio_atendimento,
          codigo_disponibilizacao: e.codigo_disponibilizacao || projeto.codigo_disponibilizacao,
          descricao: l.descricao || e.descricao || projeto.descricao_padrao || "",
        });
      });
    }
    ...
    // Recalcula horas no banco para todas as empresas participantes
    for (const empresaId of Object.keys(porEmpresa)) {
      await recalcularHorasEmpresa(empresaId);
    }
